/* Copy-to-clipboard button for the *static* Raku code examples — the ones
   without a Run button (fragments, Raku++ gaps, environment-dependent programs,
   marked `.no-run` by the generator). Runnable blocks get their own Copy button
   from the Raku.js editor that replaces them, and program output (```console)
   is not `.raku`, so neither gets a duplicate button here. */
(function () {
  'use strict';
  ...
  function addButtons() {
    var blocks = document.querySelectorAll('div.highlight.raku.no-run');
